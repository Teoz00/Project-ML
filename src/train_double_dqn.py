...
def main():
    ...
    agent = DoubleDQNAgent(n_states, n_actions, device=device, lr=c['lr'], gamma=c['gamma'],
                           buffer_size=c['buffer_size'], batch_size=c['batch_size'],
                           target_update_every=c['target_update_every'],
    ...
    ensure_dir(save_dir)
    save_json({'config': c, 'seed': seed}, os.path.join(save_dir, "params.json"))

    tb_writer = None
    if c.get('tensorboard', False):
        tb_dir = os.path.join(c.get('tb_logdir','tb_logs/double_dqn'), f"seed_{seed}")
    ...
    for ep in range(1, c['episodes']+1):
        ...
        if ep % 100 == 0:
            ...
            print(f"[DoubleDQN] Ep {ep}/{c['episodes']} last100_avg {last100:.2f}")
            if tb_writer:
    ...
    plot_learning_curve(rewards, filename=os.path.join(save_dir, "learning_curve.png"), title="Double DQN on Taxi-v3")
    if tb_writer:
        tb_writer.close()
    env.close()
