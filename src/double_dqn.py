...
class DoubleDQNAgent:
    def __init__(self, n_states, n_actions, device='cpu', lr=1e-3, gamma=0.99,
        ...
        self.policy_net = DQNNetwork(n_states, n_actions, embedding_dim=embedding_dim, hidden_dim=hidden_dim).to(device)
        self.target_net = DQNNetwork(n_states, n_actions, embedding_dim=embedding_dim, hidden_dim=hidden_dim).to(device)
    ...
    def save(self, path):
        ensure_dir(path)
        torch.save(self.policy_net.state_dict(), os.path.join(path, "double_dqn.pt"))
        torch.save(self.target_net.state_dict(), os.path.join(path, "double_dqn_target.pt"))
