...
class DQNAgent:
    def __init__(self, n_states, n_actions, device='cpu', lr=1e-3, gamma=0.99,
                 buffer_size=10000, batch_size=64, epsilon_start=1.0, epsilon_end=0.05, epsilon_decay=20000,
    ...
    def learn(self):
        ...
        with torch.no_grad():
            q_next = self.policy_net(s2)  # single net variant
    ...
    def save(self, path):
        ensure_dir(path)
        torch.save(self.policy_net.state_dict(), os.path.join(path, "dqn.pt"))
